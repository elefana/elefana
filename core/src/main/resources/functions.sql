...
CREATE SEQUENCE IF NOT EXISTS elefana_dup_key_id MINVALUE -9223372036854775807 START -9223372036854775807 CYCLE;
...
CREATE TABLE IF NOT EXISTS elefana_field_stats_field (_fieldName VARCHAR(255) PRIMARY KEY, _type VARCHAR(255));
CREATE TABLE IF NOT EXISTS elefana_field_stats_index (_indexName VARCHAR(255) PRIMARY KEY, _maxDocs BIGINT, _timestamp BIGINT);
CREATE TABLE IF NOT EXISTS elefana_field_stats_fieldstats (_indexName VARCHAR(255), _fieldName VARCHAR(255), _minValue TEXT, _maxValue TEXT, _docCount BIGINT, _sumDocFreq BIGINT, _sumTotalTermFreq BIGINT, CONSTRAINT _fieldstats_pk PRIMARY KEY (_indexname, _fieldname));
