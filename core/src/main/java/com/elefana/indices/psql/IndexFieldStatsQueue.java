...
public class IndexFieldStatsQueue extends HashPsqlBackedQueue<QueuedIndex> {
	...
	protected void fetchFromDatabaseUnique(JdbcTemplate jdbcTemplate, List<QueuedIndex> results, int from, int limit) throws SQLException {
		final SqlRowSet rowSet = jdbcTemplate.queryForRowSet("SELECT * FROM elefana_index_field_stats_queue WHERE _timestamp <= " +
	...
	public void appendToDatabase(JdbcTemplate jdbcTemplate, List<QueuedIndex> elements) throws SQLException {
		for(QueuedIndex queuedIndex : elements) {
			jdbcTemplate.execute("INSERT INTO elefana_index_field_stats_queue (_index, _timestamp) VALUES ('" +
